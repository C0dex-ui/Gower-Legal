...
name: motion-design
description: >
  CSS motion taste for homepage-generator mockups: timing, easing, stagger.
  MUST Read HOUSE.md first. CSS only. No GSAP, no Lottie JSON, no MCP, no
  scroll-pin, no infinite bounce. Triggers: homepage motion, DESIGN.md motion
  tokens, reveal, hover, prefers-reduced-motion.
license: MIT
metadata:
  author: LottieFiles
  version: "1.0.0"
...
**This repo:** Read [HOUSE.md](HOUSE.md) first. Implement in CSS only.
